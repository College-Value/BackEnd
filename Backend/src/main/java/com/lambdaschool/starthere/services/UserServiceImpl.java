...
public class UserServiceImpl implements UserDetailsService, UserService
{
    ...
    public User save(User user)
    {
        ...
        for (Searches s : user.getSavedSearches())
        {
            newUser.getSavedSearches()
                    .add(new Searches(s.getCollegename(), s.getCity(), s.getState(),
                            s.getEarnings(), s.getDegree(), newUser));
        }
                    ...
                    .size() > 0)
            {
                for (Searches s : user.getSavedSearches())
                {
                    currentUser.getSavedSearches()
                            .add(new Searches(s.getCollegename(), s.getCity(), s.getState(),
                                    s.getEarnings(), s.getDegree(), currentUser));
                }
            }
